...
pub struct UserPrefs {
    ...
    // Legacy field — kept for deserialization compatibility, ignored at runtime
    #[allow(dead_code)]
    #[serde(default, skip_serializing)]
    git_diff_custom_n: u32,
}
...
mod prefs_tests {
    ...
    #[test]
    fn old_prefs_with_custom_n_still_deserializes() {
        // Old prefs had git_diff_custom_n — verify it doesn't break deserialization
        let old_json = r#"{"theme":"calm","color_mode":"TdgGrade","size_mode":"lines","scale_mode":"smooth","layout_mode":"treemap","edge_filter":"all","show_all_edges":false,"activity_panel_open":false,"last_root_path":null,"git_diff_window":{"TimeSecs":86400},"git_diff_custom_n":10}"#;
        let prefs: UserPrefs = serde_json::from_str(old_json).expect("should handle old custom_n field");
        assert_eq!(prefs.git_diff_window, DiffWindow::TimeSecs(86400));
    }

    #[test]
    fn git_diff_window_commit_range_roundtrips() {
        let prefs = UserPrefs {
            git_diff_window: DiffWindow::CommitRange {
        ...
        let json = serde_json::to_string(&prefs).expect("serialize");
        let restored: UserPrefs = serde_json::from_str(&json).expect("deserialize");
        assert_eq!(prefs.git_diff_window, restored.git_diff_window);
    }
}
