...
/// Draw GSD phase association section for the selected file (Plan 03 enhanced version).
///
/// Shows phase number, name, goal, and status for the primary phase. Also shows
/// "Also modified in: Phase X, Y" if the file appears in multiple phases.
...
fn draw_gsd_phase_section(ui: &mut Ui, state: &AppState, path: &str) {
    ...
    .id_salt("gsd_phase_detail")
    .default_open(true)
    .show(ui, |ui| {
        let Some(report) = &state.gsd_phase_report else {
            ui.label(
                egui::RichText::new("No phase data")
                    .monospace()
                    .size(9.0)
                    .color(ui.visuals().weak_text_color()),
            );
        ...
        ui.horizontal(|ui| {
            ui.colored_label(
                status_color,
                egui::RichText::new(format!("Phase {}: {}", phase.number, phase.name))
                    .monospace()
                    .size(9.0)
                    .strong(),
            );
        });
        ui.label(egui::RichText::new(format!("Goal: {}", phase.goal)).monospace().size(9.0)
            .color(ui.visuals().weak_text_color()));
        ui.horizontal(|ui| {
        ...
        if !other_phases.is_empty() {
            ui.label(
                egui::RichText::new(format!("Also modified in: Phase {}", other_phases.join(", Phase ")))
                    .monospace()
                    .size(9.0)
                    .color(ui.visuals().weak_text_color()),
            );
